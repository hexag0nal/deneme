...
def getMeaning(word):
    if word in data:
        ...
        return word, ["Bu kelime sözlükte bulunamadı."], -1
...
def translate(query):
    ...
    if check == 1:
        return ["\"" + word.capitalize() + "\" kelimesinin tanımı:", result]
    elif check == 0:
        return ["Sanırım aradığınız kelime \"" + word.capitalize() + "\"", "Bu kelimenin tanımı:\n" + result]
    else:
        return [result, '']
